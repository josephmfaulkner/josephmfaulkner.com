...
resource "aws_codepipeline" "static_web_pipeline" {
  name     = "josephmfaulkner.com-pipeline"
  ...
  stage {
    ...
    action {
      category = "Deploy"
      configuration = {
        "BucketName" = aws_s3_bucket.test_site_bucket_name.id
        "Extract"    = "true"
      }
      input_artifacts = [
        "BuildArtifact",
  ...
  stage {
    ...
    action {
      ...
      configuration = {
          ExternalEntityLink = "http://${aws_s3_bucket.test_site_bucket_name.website_endpoint}"
      }
    }
  }
